...
export function TeamCard({ members, currentUserId, ownerId }: Props) {
  return (
                  ...
                  {m.projects.length > 0 ? (
                    <div className="flex flex-wrap gap-1.5">
                      {m.projects.map((p) => (
                        ...
                        >
                          <span className="text-[11px] font-medium text-foreground-2 group-hover/proj:text-brand truncate max-w-[90px]">
                            {p.name}
                          </span>
                          {p.tag && (
