    ...
    <div className="bg-card rounded-card border-t-[3px] border-t-brand border-x border-b border-border p-4 shadow-[0_1px_4px_oklch(0%_0_0/4%)]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        <div className="flex items-center gap-3 flex-shrink-0">
          <div className="w-8 h-8 rounded-btn flex items-center justify-center bg-brand-subtle text-brand">
            <ListTodo className="w-3.5 h-3.5" />
          </div>
          <div>
            <p className="text-3xl font-bold text-brand tracking-tight tabular-nums">{kpi.total}</p>
          ...
          {stats.map((s) => {
            ...
            return (
              <div key={s.key} className="flex flex-col gap-2">
                <div className={`w-8 h-8 rounded-btn flex items-center justify-center ${s.bg} ${s.color}`}>
                  <Icon className="w-3.5 h-3.5" />
                </div>
                <div>
                  <p className="text-xl font-bold text-foreground tracking-tight tabular-nums">{kpi[s.key]}</p>
