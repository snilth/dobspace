  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-8">
      ...
      <div className="flex items-center gap-2 mb-5">
        {TABS.map((t) => {
          const count = t.key === "all" ? notifications.length : unread.length;
          return (
            <button
              key={t.key}
      ...
      {isLoading ? (
      ...
      ) : notifications.length === 0 || (tab === "unread" && unread.length === 0) ? (
        <EmptyState />
      ) : (
        <>
        {confirmClear && (
          <ConfirmClearModal
            ...
            onConfirm={() => {
              clearRead.mutate(undefined, { onSuccess: () => setConfirmClear(false) });
            }}
          />
        )}
        <div className="space-y-6">
              ...
              <div className="bg-card rounded-[12px] border border-border overflow-hidden divide-y divide-border">
                {unread.map((n) => (
                  <NotifRow
                    ...
                    onDismiss={() => dismiss.mutate({ id: n.id })}
                  />
                ))}
              </div>
            </section>
          ...
          {/* Read section — only in All tab */}
          {tab === "all" && read.length > 0 && (
            <section>
              <p className="text-[11px] font-semibold text-muted uppercase tracking-wider mb-2.5">
                Read · {read.length}
              </p>
...
function NotifRow({ notification: n, onMarkRead, onDismiss }: {
  ...
  return (
    <div className={cn(
      "flex items-start gap-3 px-4 py-3.5 hover:bg-surface-2 transition-colors group",
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-[2px]">
      <div className="w-full max-w-sm bg-card rounded-[16px] border border-border shadow-2xl p-5">
        ...
        <p className="text-[13px] text-muted mb-5">
          These notifications will be removed from your inbox. This action cannot be undone.
        </p>
